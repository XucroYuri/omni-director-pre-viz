...
async function main() {
  ...
  let hasLock = false;
  try {
    await acquireWebStackLock('test:e2e:web-core-chain');
    hasLock = true;
    ...
    console.log(`[test:e2e:web-core-chain] WEB_BASE_URL=${baseUrl}`);
    ...
    await runCommand(['--prefix', 'apps/web', 'run', 'e2e:core-chain'], { env: childEnv });
  } finally {
...
main().catch((error) => {
  console.error('Web Core Chain E2E failed', error);
  process.exitCode = 1;
});
